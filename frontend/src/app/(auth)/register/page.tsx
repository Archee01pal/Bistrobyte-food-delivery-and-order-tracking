...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Backend register API failed or offline, proceeding with local session setup.');
      }

      localStorage.setItem('user', JSON.stringify(userData));

      if (setUser) setUser(userData as any);
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-gradient-to-br from-amber-100 via-teal-50 to-sky-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-10 right-10 w-80 h-80 bg-amber-400/25 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-80 h-80 bg-cyan-400/25 rounded-full blur-3xl pointer-events-none" />

      {/* Main Form Container with Amber Border */}
      <div className="w-full max-w-md bg-white/90 backdrop-blur-xl border-2 border-amber-400/80 p-8 rounded-3xl shadow-2xl relative z-10 hover:border-amber-500 hover:shadow-amber-500/20 transition-all duration-300">
        <div className="text-center mb-6">
          ...
          <p className="text-xs font-semibold text-slate-500 mt-1">
            Join BistroByte and start exploring your favorite restaurants
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Clean Amber Sign Up Button with Crisp White Text */}
          <button
            type="submit"
            disabled={loading}
        ...
        <p className="mt-6 text-center text-xs font-medium text-slate-600 border-t border-slate-200/80 pt-4">
          Already have an account?{' '}
          <Link href="/login" className="font-bold text-amber-600 hover:text-teal-600 hover:underline transition-colors">
            Log in
          </Link>
        </p>
      </div>
